  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      ...
      try {
        ...
        const roadmapRes = await axios.get('http://localhost:8080/api/dashboard/roadmap', {
          headers: { Authorization: `Bearer ${token}` }
        });
        setRoadmap(roadmapRes.data);

        const mistakesRes = await axios.get('http://localhost:8080/api/aptitude/analytics/mistakes', {
          headers: { Authorization: `Bearer ${token}` }
        });
