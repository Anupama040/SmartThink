...
export default function HRInterview() {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      ...
      try {
        const res = await axios.get(`http://localhost:8080/api/hr/interview?category=${category}`, {
  ...
  const handleNext = async () => {
    if (currentIdx < questions.length - 1) {
      ...
      try {
        ...
        const res = await axios.post('http://localhost:8080/api/hr/evaluate', payload, {
            headers: { Authorization: `Bearer ${token}` }
        });
